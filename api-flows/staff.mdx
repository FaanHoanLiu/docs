...
The server responds with a ```200``` status code with the response body containing a list of a dish's ingredients.
...
The server responds with a ```200``` status code with the response body containing the updated order resource. For takeaway orders, the expeditor changes the ```status``` to ```Ready for Pickup``` to indicate the order is ready for the patron.